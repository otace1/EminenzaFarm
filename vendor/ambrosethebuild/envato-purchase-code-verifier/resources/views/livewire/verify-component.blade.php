    ...
    <form wire:submit.prevent="verifyPurchase">
        ...
        <div>
            ...
            <div class="flex items-center pl-6 border border-gray-300 bg-white rounded-full">
                ...
                <input class="w-full pr-6 pl-4 py-4 font-bold placeholder-gray-400 rounded-r-full focus:outline-none" type="text" placeholder="Purchase Code" wire:model='purchase_code'>
            </div>
            @error('purchase_code')
            ...
            <div class="mt-3 flex items-center pl-6 border border-gray-300 bg-white rounded-full">
                ...
                <input class="w-full pr-6 pl-4 py-4 font-bold placeholder-gray-400 rounded-r-full focus:outline-none" type="text" placeholder="Buyers Username" wire:model='buy_username'>
            </div>
            @error('buy_username')
            <div class="mt-1 text-xs text-red-700 text-center">{{ $message }}</div>
            @enderror
            ...
            {{-- email for reminder --}}
            <input class="mt-3 w-full rounded-xl font-bold placeholder-gray-400 focus:outline-none" type="text" placeholder="Alert Email" wire:model='alert_email' />
            @error('alert_email')
            <div class="mt-1 text-xs text-red-700 text-center">{{ $message }}</div>
            @enderror
        ...
        <button class="mt-6 py-4 w-full bg-green-500 hover:bg-green-600 text-white font-bold rounded-full transition duration-200">Get
            started</button>
